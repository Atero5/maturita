...
if ($method === 'POST') {
    ...
    if ($task && $end_time) {
        $stmt = $conn->prepare('INSERT INTO tasks (user_id, task, end_time) VALUES (?, ?, ?)');
...
if ($method === 'GET') {
    $stmt = $conn->prepare('SELECT taskId, task, end_time, date FROM tasks WHERE user_id = ? ORDER BY taskId DESC');
